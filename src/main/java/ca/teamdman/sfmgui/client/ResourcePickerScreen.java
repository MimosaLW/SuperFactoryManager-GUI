...
public class ResourcePickerScreen extends Screen {
    ...
    private void renderTypeTags(GuiGraphicsExtractor graphics, int mx, int my) {
        int[][] tl = tagLayout();
        for (int i = 0; i < TYPE_TAGS.size(); i++) {
            ...
            graphics.centeredText(this.font, TYPE_TAGS.get(i).label(),
                    tx + tw / 2, TAG_Y + 4, hover ? 0xFFFFFFFF : 0xFFCCE0FF);
        }
    }

    private void renderGrid(GuiGraphicsExtractor graphics, int mx, int my) {
        int left = gridLeft();
        int top = gridTop();
