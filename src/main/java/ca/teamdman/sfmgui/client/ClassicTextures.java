...
public final class ClassicTextures {
    ...
    private static Identifier tex(String name) {
        return Identifier.fromNamespaceAndPath("sfmgui", "textures/gui/classic/" + name + ".png");
    }

    public static final Identifier FLOW = tex("flow_components");
    ...
    public static void flow(GuiGraphicsExtractor g, int x, int y, int srcX, int srcY, int w, int h) {
        g.blit(RenderPipelines.GUI_TEXTURED, FLOW, x, y, srcX, srcY, w, h, ATLAS, ATLAS);
    }
     ...
     */
    public static void nodeFrameSized(GuiGraphicsExtractor g, int x, int y, int w, int h) {
    ...
    private static void nineSlice(
            GuiGraphicsExtractor g, Identifier tex,
    ...
    private static void blitRegion(GuiGraphicsExtractor g, Identifier tex, int x, int y, int dw, int dh, int u, int v, int sw, int sh) {
        if (dw <= 0 || dh <= 0) {
            return;
        }
        g.blit(RenderPipelines.GUI_TEXTURED, tex, x, y, u, v, dw, dh, sw, sh, ATLAS, ATLAS);
    }

    /** Expand/collapse arrow at a node's top-right; col by large, row by hover. */
    public static void nodeArrow(GuiGraphicsExtractor g, int x, int y, boolean large, boolean hover) {
    ...
    /** A menu title strip. */
    public static void menuStrip(GuiGraphicsExtractor g, int x, int y) {
        flow(g, x, y, MENU_ITEM_SRC_X, MENU_ITEM_SRC_Y, MENU_ITEM_W, MENU_ITEM_H);
    }

    /** Menu fold arrow; col by hover, row by open. */
    public static void menuArrow(GuiGraphicsExtractor g, int x, int y, boolean open, boolean hover) {
     ...
     */
    public static void connection(GuiGraphicsExtractor g, int x, int y, boolean input, boolean hover) {
    ...
    public static void toolbarButton(GuiGraphicsExtractor g, int x, int y, int iconIndex, boolean hover) {
    ...
    public static void toolbarFrame(GuiGraphicsExtractor g, int x, int y, boolean hover) {
    ...
    public static void wideButton(GuiGraphicsExtractor g, int x, int y, int w, boolean hover) {
